...
import { Zap, Battery, Car, Thermometer, Calendar, Home, X } from 'lucide-react';
...
const SystemTimeline: React.FC<SystemTimelineProps> = ({ systems, houses }) => {
  ...
  const getTimeActive = () => {
    if (filteredSystems.length === 0) return { value: 0, unit: 'days' };

    // Find the oldest system install date
    const oldestDate = filteredSystems.reduce((oldest, system) => {
      const systemDate = new Date(system.install_date);
      return systemDate < oldest ? systemDate : oldest;
    }, new Date(filteredSystems[0].install_date));
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                {activeFilter ? `${getFilterDisplayName(activeFilter)} Systems` : 'Total Systems'}
              </div>
            </div>
            <div>
            ...
            <div>
              <div className="text-2xl font-bold text-primary">
                £{filteredSystems.reduce((total, system) => total + (system.system_cost || 0), 0).toLocaleString()}
              </div>
